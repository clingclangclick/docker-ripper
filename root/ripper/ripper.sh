...
CHOWN_OWNER=${CHOWN_OWNER:-nobody}
CHOWN_GROUP=${CHOWN_GROUP:-users}
...
while true; do
	...
	if (($BAD_RESPONSE >= $BAD_THRESHOLD)); then
		...
		if eject -v $DRIVE | grep 'whole-disk' >>/dev/null; then
			...
			pushover "First attempt at Ejecting Disk Succeeded"
			exit 1
		else
			...
			pushover "$(date "+%d.%m.%Y %T") : First Attempt At Ejecting Disk Failed. Trying Alternative Method."
			sleep 2
	...
	if [ "$DVD" = 'DRV:0,2,999,1,"' ]; then
		...
		# permissions
		chown -R $CHOWN_OWNER:$CHOWN_GROUP "$STORAGE_DVD" && chmod -R g+rw "$STORAGE_DVD"
	fi
	...
	if [ "$CD1" = 'DRV:0,2,999,0,"' ]; then
		if [ "$CD2" = '","","'$DRIVE'"' ]; then
			...
			# permissions
			chown -R $CHOWN_OWNER:$CHOWN_GROUP "$STORAGE_CD" && chmod -R g+rw "$STORAGE_CD"
		else
			...
			echo "$(date "+%d.%m.%Y %T") : Done! Ejecting Disk"
			pushover "$(date "+%d.%m.%Y %T") : Done! Ejecting Disk"
			if eject -v $DRIVE | grep 'whole-disk' >>/dev/null; then
				echo "First attempt at Ejecting Disk Succeeded"A
				pushover "First attempt at Ejecting Disk Succeeded"A
			else
				echo "First Attempt At Ejecting Disk Failed. Attempting Alternative Method." >>$LOGFILE 2>&1
				echo "$(date "+%d.%m.%Y %T") : First Attempt At Ejecting Disk Failed. Trying Alternative Method."
				pushover "$(date "+%d.%m.%Y %T") : First Attempt At Ejecting Disk Failed. Trying Alternative Method."
			...
			# permissions
			chown -R $CHOWN_OWNER:$CHOWN_GROUP "$STORAGE_DATA" && chmod -R g+rw "$STORAGE_DATA"
		fi
	fi
	# Wait a minute
	sleep 1m
